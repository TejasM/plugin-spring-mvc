...
@RequiresFacet({DependencyFacet.class,
        ...
        SpringFacet.class})
public class SpringScaffoldX extends BaseFacet implements ScaffoldProvider {
    ...
    @Inject
    public SpringScaffoldX(final Configuration config,
    ...
    public List<Resource<?>> setup(String targetDir, boolean overwrite, boolean installTemplates) {
        ...
        result = generateTemplates(targetDir, overwrite);
        ...
        if (!targetDir.equals("/"))
        {
            result.add(org.jboss.forge.scaffold.util.ScaffoldUtil.createOrOverwrite(this.prompt, web.getWebResource("WEB-INF/views/index.jsp"),
                    this.indexTemplate.render(context), overwrite));
        }

        // Basic pages

        result.add(org.jboss.forge.scaffold.util.ScaffoldUtil.createOrOverwrite(this.prompt, web.getWebResource("WEB-INF/views" + targetDir + "index.jsp"),
                this.indexTemplate.render(context), overwrite));

        result.add(org.jboss.forge.scaffold.util.ScaffoldUtil.createOrOverwrite(this.prompt, web.getWebResource("WEB-INF/views/error.jsp"),
                this.errorTemplate.render(context), overwrite));

        // Static resources - only add them if they are not already present.

        org.jboss.forge.scaffold.util.ScaffoldUtil.createOrOverwrite(this.prompt, web.getWebResource("/resources/bootstrap.css"),
                getClass().getResourceAsStream("/scaffold/spring/bootstrap.css"), overwrite);
        org.jboss.forge.scaffold.util.ScaffoldUtil.createOrOverwrite(this.prompt, web.getWebResource("/resources/false.png"),
                getClass().getResourceAsStream("/scaffold/spring/false.png"), overwrite);
        org.jboss.forge.scaffold.util.ScaffoldUtil.createOrOverwrite(this.prompt, web.getWebResource("/resources/forge-logo.png"),
                getClass().getResourceAsStream("/scaffold/spring/forge-logo.png"), overwrite);
        org.jboss.forge.scaffold.util.ScaffoldUtil.createOrOverwrite(this.prompt, web.getWebResource("/resources/forge-style.css"),
                getClass().getResourceAsStream("/scaffold/spring/forge-style.css"), overwrite);
        org.jboss.forge.scaffold.util.ScaffoldUtil.createOrOverwrite(this.prompt, web.getWebResource("/resources/search.png"),
                getClass().getResourceAsStream("/scaffold/spring/search.png"), overwrite);
        org.jboss.forge.scaffold.util.ScaffoldUtil.createOrOverwrite(this.prompt, web.getWebResource("/resources/true.png"),
                getClass().getResourceAsStream("/scaffold/spring/true.png"), overwrite);
        org.jboss.forge.scaffold.util.ScaffoldUtil.createOrOverwrite(this.prompt, web.getWebResource("/resources/jboss-community.png"),
    ...
    public List<Resource<?>> generateFrom(List<Resource<?>> resource, String targetDir, boolean overwrite) {
        ...
        List<JavaClass> entities = new ArrayList<JavaClass>();

        List<Resource<?>> result = new ArrayList<Resource<?>>();

        for (Resource entity : resource) {
            try {
                if (((JavaResource) entity).getJavaSource().getAnnotation(Entity.class) != null) {
                    entities.add((JavaClass) (((JavaResource) entity).getJavaSource()));
                }
    ...
    public TemplateStrategy getTemplateStrategy() {
        return new SpringTemplateStrategyX(this.project);
    }
    ...
    protected Resource<?> generateNavigation(String targetDir, final boolean overwrite)
        ...
        Writer writer = new StaticUtils.IndentedWriter(new StringWriter(), this.navigationTemplateIndent);
        ...
        if (targetDir.equals("/")) {
            return org.jboss.forge.scaffold.util.ScaffoldUtil.createOrOverwrite(this.prompt, (FileResource<?>) getTemplateStrategy().getDefaultTemplate(),
                    this.navigationTemplate.render(context), overwrite);
        } else {
            return org.jboss.forge.scaffold.util.ScaffoldUtil.createOrOverwrite(this.prompt, web.getWebResource("WEB-INF/layouts/" + targetDir.substring(1,
